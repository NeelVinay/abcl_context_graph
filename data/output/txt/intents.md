...
| `customer_acknowledge` | Customer acknowledgement / agreement / back-channel | 301 |
...
| `agent_greet` | Agent's opening / identifies the business | 219 |
| `agent_ask_to_repeat` | Agent asks the customer to repeat / didn't catch it | 216 |
| `customer_greet` | Customer's opening / confirms who they are | 214 |
...
| `customer_request_wait` | Customer asks the agent to hold on a moment | 160 |
...
| `agent_acknowledge` | Agent acknowledgement / back-channel (ok, achha, noted) | 51 |
...
| `customer_express_distrust` | Customer suspects fraud / is reluctant to share info | 30 |
...
| `agent_reassure_trust` | Agent reassures the customer it's genuine (not fraud) | 27 |
...
| `agent_clarify` | Agent clarifies or re-explains a step | 16 |
