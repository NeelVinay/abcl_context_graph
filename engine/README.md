...
**This file documents the internals**, and lives inside `engine/` alongside the
...
three commands, run from the repo root (not from inside `engine/`):

```bash
./load-transcripts ~/Downloads/your_calls    # pulls transcripts into INPUT/
./load-prompt ~/Downloads/your_prompt.raven   # pulls the prompt into INPUT/
./run-pipeline                                 # context graph -> analysis -> OUTPUT/
```

`INPUT/` and `OUTPUT/` both live at the **repo root**, one level up from this
file — `engine/` holds only code and internal working data, never the thing you
actually look at day to day. `OUTPUT/` always holds the result of the latest run;
...
shown from here on assumes you've `cd`'d into `engine/` first.
...
This is what `./load-transcripts` + the context-graph half of `./run-pipeline` call
...
3. Write everything to `data/output/<client>_output/` — an internal, per-client
   working area; `./run-pipeline` copies the currently-active client's result into
   the single `OUTPUT/Context_Graph/` folder for you.
...
```
commands.md          The normal workflow: 3 commands. Start here.

INPUT/               Drop your transcripts and .raven prompt here.
  transcripts/
  prompt/
OUTPUT/               Result of the latest ./run-pipeline run. Rebuilt every time.
  Context_Graph/

load-transcripts     Registers INPUT/transcripts/. No graph build.
load-prompt           Validates INPUT/prompt/. No analysis.
run-pipeline           Everything, start to finish, into OUTPUT/.
improve <path>         Advanced: improve an arbitrary prompt file directly,
                        bypassing INPUT/OUTPUT/.
...
data/                 Internal working state — not where you look day to day;
  ...
  output/<name>_output/ Internal per-client working area, one per client that's
                         ever been processed. ./run-pipeline copies the currently-
                         active client's result into OUTPUT/Context_Graph/.
...
Normal use is `./run-pipeline` (see `commands.md`) — no API key needed, it uses
...
- `INPUT/transcripts/` and `data/clients/*/transcripts/` contain real customer
