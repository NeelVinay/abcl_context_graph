...
```bash
./bin/load-transcripts ~/Downloads/your_calls        # pull transcripts in
./bin/load-prompt ~/Downloads/your_prompt.raven       # pull the prompt in
./bin/run-pipeline                                     # graph -> analysis -> deliverables/
```
...
```
bin/                     Executable commands. Run these three, in order.
  load-transcripts          pull transcripts into intake/, register them
  load-prompt                pull the prompt into intake/, validate it
  run-pipeline                 context graph -> analysis -> deliverables/

intake/                  Drop your files here.
...
platform/                All code, dependencies, and internal data.
  ...
  improve                  Advanced/testing tool — improves an arbitrary prompt
                            file directly, bypassing intake/deliverables.
...
`intake/` is never modified by any command; only its contents are gitignored
(the folders themselves persist via `.gitkeep`, so a fresh clone still has
somewhere to drop files). `deliverables/` is fully rebuilt on every
`./bin/run-pipeline` run — it never holds a mix of old and new results, and
there is only ever one active client's data in it at a time.
...
which is why both `intake/transcripts/` and the internal `platform/data/`
transcript stores are gitignored rather than committed. See
**[`platform/README.md`](platform/README.md#security-and-pii)** for the full
detail on what is and isn't sent anywhere, and the compliance guards that
check every generated line before it can be written.
...
- **[`platform/README.md`](platform/README.md)** — full technical documentation:
