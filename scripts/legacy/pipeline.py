"""RETIRED: superseded by run_client.py, which auto-detects the client and writes
to a single data/output/<client>_output/ folder instead of the two fixed dataset
names ("mp3" / "txt") this module hardcodes. Kept for reference; run via
`python -m scripts.legacy.make_flowchart` / `make_graph`, not by path.

Shared helper for the two viz entry points (make_flowchart.py / make_graph.py).

Dataset selector:
  mp3  -> transcripts in data/clients/justdial/transcripts.
  txt  -> transcripts in data/clients/abcl/transcripts.
