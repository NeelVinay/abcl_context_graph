...
def _relevant_states_text(d, pack, max_chars: int = 9000) -> str:
    ...
    Originally stopped at sms_send — everything after the SMS is sent (the
    SECOND-worst stage in every ABCL run so far, ~50% incomplete) was invisible.
    With no valid target in view for that stage, proposals meant for it kept
    landing on sms_send itself, which is a fixed single line and rejects any
    addition outright — silently losing whole themes (process_mechanics, the
    single largest post-pitch theme, and already_engaged) every run. Extended
    past the SMS send so those themes have somewhere real to go."""
    wanted = ["start", "self_intro", "loan_intro", "loan_intro_persuade", "sms_send",
              "web_journey_start", "apply_now", "handle_amount_more",
              "handle_security_concern", "handle_agent_query",
