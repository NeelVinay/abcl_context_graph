...
**This file documents the internals**, and lives inside `platform/` alongside the
code it describes. For day-to-day use you don't need anything below this
paragraph — see **[`../COMMANDS.md`](../COMMANDS.md)** at the repo root instead:
three commands, run from the repo root (not from inside `platform/`):

```bash
./bin/load-transcripts ~/Downloads/your_calls    # pulls transcripts into intake/
./bin/load-prompt ~/Downloads/your_prompt.raven   # pulls the prompt into intake/
./bin/run-pipeline                                 # context graph -> analysis -> deliverables/
```

`intake/` and `deliverables/` both live at the **repo root**, one level up from this
file — `platform/` holds only code and internal working data, never the thing you
actually look at day to day. `deliverables/` always holds the result of the latest run;
neither `intake/` file is ever modified.

Everything below this point documents what's actually happening underneath —
useful for finer control, onboarding a new client, or understanding the pipeline,
but not needed for the routine "improve this prompt" workflow. Every command
shown from here on assumes you've `cd`'d into `platform/` first.
...
This is what `./bin/load-transcripts` + the context-graph half of `./bin/run-pipeline` call
underneath. Point it at a folder of transcripts and it will:
...
2. Use that client's own taxonomy and trained model if one exists (currently ABCL,
   ...
   new.
3. Write everything to `data/output/<client>_output/` — an internal, per-client
   working area; `./bin/run-pipeline` copies the currently-active client's result into
   the single `deliverables/Context_Graph/` folder for you.

By default, the transcripts given are treated as that client's full, current set
and replace whatever was there before. Pass `--append` to add the new transcripts
to the client's existing set instead of replacing it.
...
This file lives in `platform/`, so everything below is relative to it. The
repo-root layout (one level up) is:

```
bin/                  The 3 commands. Start here — see ../COMMANDS.md.
  load-transcripts       Registers intake/transcripts/. No graph build.
  load-prompt             Validates intake/prompt/. No analysis.
  run-pipeline              Everything, start to finish, into deliverables/.

intake/               Drop your transcripts and .raven prompt here.
  transcripts/
  prompt/
deliverables/         Result of the latest ./bin/run-pipeline run. Rebuilt every time.
  Context_Graph/

platform/             Everything below — code, dependencies, internal data.
  improve <path>         Advanced: improve an arbitrary prompt file directly,
                          bypassing intake/deliverables/.
  reset <client>          Wipes a client's loaded data back to zero (archived,
                           never deleted) — for proving run-pipeline is genuine.
```

And inside `platform/` itself:

```
run_client.py       The lower-level entry point described above.
...
data/                 Internal working state — not where you look day to day;
                      see intake/ and deliverables/ instead.
  clients/<name>/transcripts/   The persistent transcript store per client, kept
                                 in sync with intake/transcripts/ by
                                 ./bin/load-transcripts. Multiple clients' data lives
                                 here (ABCL, JustDial, Myntra, from earlier
                                 onboarding) but only ONE is ever the active
                                 intake/deliverables set at a time.
  ...
  output/<name>_output/ Internal per-client working area, one per client that's
                         ever been processed. ./bin/run-pipeline copies the currently-
                         active client's result into deliverables/Context_Graph/.
  ...
  .reset-archive/        ./reset's own safety net — never deletes, archives here.
...
Normal use is `./bin/run-pipeline` (see `COMMANDS.md`) — no API key needed, it uses
the local Claude Code CLI. The direct, lower-level interface:

```bash
# fully autonomous: transcripts + prompt in, improved prompt out
export ANTHROPIC_API_KEY=sk-ant-...   # or have Claude Code installed — either works
python run_improve.py data/clients/abcl/prompt.raven --client abcl --auto

# no LLM at all: mine mechanically and write a review queue instead
...
- `../intake/transcripts/` and `data/clients/*/transcripts/` contain real customer
  transcripts and should be treated as confidential. Do not share or upload them.
